<?php
// backend/ethiopia_service/regions/amhara/health.php

require_once __DIR__ . '/../../../../config/db.php';
require_once __DIR__ . '/../../../helpers/log.php';

function amhara_health(): array {
    $regionName = 'Amhara';
    $status     = 'OK';
    $components = [
        'db'    => 'OK',
    ...
    // ✅ Cache table check (by city_id for Amhara’s main city)
    try {
        $pdo = db();
        // Adjust city name to the one you use in your Amhara forecast service
        $stmt = $pdo->prepare("SELECT id FROM cities WHERE name = ?");
        $stmt->execute(['Bahir Dar']); // Example city for Amhara region
...
if (realpath(__FILE__) === realpath($_SERVER['SCRIPT_FILENAME'])) {
    ...
    echo json_encode(amhara_health(), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    exit;
}
