...
// South region alerts service with caching + consistent health reporting
...
function south_alerts(): array {
    ...
    if (!$apiKey) {
        return ['city' => 'Hawassa', 'alerts' => [], 'data_status' => 'NO_API_KEY'];
    }
    ...
    // 2. Fetch from API
    try {
        $alerts = getAlertsForCity('Hawassa', $apiKey, 'en');
        if (is_array($alerts) && count($alerts) > 0) {
            ...
            return ['city' => 'Hawassa', 'alerts' => $alerts, 'data_status' => 'FRESH'];
        }
        // ✅ Even if no alerts, service is healthy
        return ['city' => 'Hawassa', 'alerts' => [], 'data_status' => 'NO_ALERTS'];
    } catch (Exception $e) {
        if ($row) {
            return [
                'city'        => 'Hawassa',
