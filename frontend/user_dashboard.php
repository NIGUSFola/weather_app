...
// ✅ Fetch unified alerts from aggregator
$alertsData = [];
try {
...
<head>
    ...
    <style>
        ...
        .forecast-card { border: 1px solid #ddd; padding: 0.5rem; margin: 0.5rem; display: inline-block; border-radius: 6px; }
        .alerts-table { width: 100%; border-collapse: collapse; margin-top: 1rem; }
        .alerts-table th, .alerts-table td { border: 1px solid #ccc; padding: 0.5rem; text-align: left; }
                                    ...
                                    <?php if (!empty($info['alerts'])): ?>
                                        <ul>
                                            <?php foreach ($info['alerts'] as $alert): ?>
                                                <li><?= htmlspecialchars($alert['event']); ?> (<?= htmlspecialchars($alert['start']); ?> → <?= htmlspecialchars($alert['end']); ?>)</li>
                                            <?php endforeach; ?>
                                        </ul>
                                    <?php else: ?><em>No alerts</em><?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            <?php else: ?><p>No regional alerts available.</p><?php endif; ?>
        </div>
